...
class Tmux(object):
    ...
    def short_info(self):
        ...
        p = (self.tid, ', '.join([s.name
                                  for s in self.sessions]), self.create_time)
    ...
    def long_info(self):
        ...
        info.append(backup_fmt %
                    (log.hl(self.tid, 'bold'), len(self.sessions)))
        ...
        for s in self.sessions:
            ...
            for w in s.windows:
                ...
                w_info = win_fmt % (w.win_id, log.hl(w.name,
                                                     'blue'), len(w.panes))
                ...
                for p in w.panes:
                    ...
                    info.append(
                        tree_struc(p_info, [is_last_s, is_last_w, is_last_p],
                                   lvl=3))
        return info
...
class Pane(object):
    ...
    def idstr(self):
        return self.sess_name + ':' + str(self.win_id) + '.' + str(
            self.pane_id)
