...
CMD_LIST_SESSIONS = config.CMD_SEP.join([
    'tmux', 'list-sessions',
    '-F#S:=:(#{window_width},#{window_height}):=:#{session_attached}'
])
#tmux list-panes -t {session}:{windowIdx}
CMD_LIST_PANES = config.CMD_SEP.join([
    'tmux', 'list-panes', '-t%s:%s',
    '-F#{pane_index}:=:(#{pane_width},#{pane_height}):=:#{pane_current_path}:=:#{pane_active}'
])
CMD_CREATE_SESSION = config.CMD_SEP.join(
    ['tmux', 'new-session', '-d', '-s%s', '-x%d', '-y%d'])
...
CMD_CAPTURE_PANE = config.CMD_SEP.join(
    ['tmux', 'capture-pane', '-%sp', '-S-100000', '-t%s'])
CMD_SHOW_OPTION = config.CMD_SEP.join(['tmux', 'show-options', '-gv', '%s'])
CMD_HAS_SESSION = config.CMD_SEP.join(['tmux', 'has-session', '-t%s'])
CMD_SET_PANE_PATH = config.CMD_SEP.join(
    ['tmux', 'send-keys', '-t%s', 'builtin cd \"%s\"\nclear\n'])
CMD_CLEAR_PANE = config.CMD_SEP.join(['tmux', 'clear-history', '-t%s'])
CMD_LIST_WINDOWS = config.CMD_SEP.join([
    'tmux', 'list-windows',
    '-F#{window_index}:=:#{window_name}:=:#{window_active}:=:#{window_layout}',
    '-t%s'
])
CMD_MOVE_WINDOW = config.CMD_SEP.join(['tmux', 'move-window', '-s%s', '-t%s'])
CMD_RENAME_WINDOW = config.CMD_SEP.join(
    ['tmux', 'rename-window', '-t%s:%d', '%s'])
CMD_NEW_EMPTY_WINDOW = config.CMD_SEP.join(
    ['tmux', 'new-window', '-d', '-t%s:%d'])
CMD_ACTIVE_WINDOW = config.CMD_SEP.join(['tmux', 'select-window', '-t%s:%d'])
CMD_SPLIT_WINDOW = config.CMD_SEP.join(
    ['tmux', 'split-window', '-d', '-l3', '-t%s:%d.%d'])
CMD_SET_LAYOUT = config.CMD_SEP.join(
    ['tmux', 'select-layout', '-t%s:%d', '%s'])
CMD_LOAD_CONTENT = config.CMD_SEP.join(
    ['tmux', 'send-keys', '-t%s', 'cat   %s\n'])
